...
const formatCommentDate = (date) => {
  ...
  commentDate.setHours(commentDate.getHours() - 1);
...
exports.getAllVideoBlobs = async (req, res) => {
  try {
    ...
    const videos = result.recordset.map((video) => {
      ...
      video.video_date = formatVideoDate(video.video_date);
      return video;
    });
...
exports.recordView = async (req, res) => {
  ...
  try {
    ...
    const recentView = await pool
      .request()
      .input("videoId", mssql.Int, videoId)
      .input("userId", mssql.VarChar, userId).query(`
    ...
    if (recentView.recordset.length === 0) {
      await pool
        ...
        .query(
          "INSERT INTO video_views (video_id, user_id, view_date) VALUES (@videoId, @userId, GETDATE())",
        );
    }

    const viewCount = await pool.request().input("videoId", mssql.Int, videoId)
      .query(`
...
exports.interactWithVideo = async (req, res) => {
  ...
  try {
    ...
    if (interactionType === "REMOVE") {
      ...
      const existingInteraction = await pool
        .request()
        .input("videoId", mssql.Int, videoId)
        .input("userId", mssql.VarChar, userId).query(`
      ...
      if (existingInteraction.recordset.length > 0) {
        await pool
          ...
          .input("videoId", mssql.Int, videoId)
          .input("userId", mssql.VarChar, userId)
          .input("interactionType", mssql.VarChar, interactionType).query(`
    ...
    const interactionCounts = await pool
      .request()
      .input("videoId", mssql.Int, videoId).query(`
                SELECT 
...
exports.getSubscriptionStatus = async (req, res) => {
  ...
  try {
    ...
    const subscriptionResult = await pool
      .request()
      .input("subscriber_id", mssql.NVarChar, currentUser)
      .input("channel_id", mssql.NVarChar, userName)
      .query(
        "SELECT * FROM subscriptions WHERE subscriber_id = @subscriber_id AND channel_id = @channel_id",
      );
    ...
    const countResult = await pool
      ...
      .input("channel_id", mssql.NVarChar, userName)
      .query(
        "SELECT COUNT(*) AS subscriberCount FROM subscriptions WHERE channel_id = @channel_id",
      );
  ...
  try {
    ...
    if (subscriptionResult.recordset.length > 0) {
      await pool
        ...
        .input("channel_id", mssql.NVarChar, channelId)
        .query(
          "DELETE FROM subscriptions WHERE subscriber_id = @subscriber_id AND channel_id = @channel_id",
        );
      ...
      await pool
        ...
        .input("channel_id", mssql.NVarChar, channelId)
        .query(
          "INSERT INTO subscriptions (subscriber_id, channel_id, subscription_date) VALUES (@subscriber_id, @channel_id, GETDATE())",
        );
...
exports.getVideoInteractions = async (req, res) => {
  ...
  try {
    ...
    if (userId) {
      ...
      const userInteractionResult = await pool
        .request()
        .input("videoId", mssql.Int, videoId)
        .input("userId", mssql.VarChar, userId).query(`
...
exports.getVideoBlob = async (req, res) => {
  ...
  try {
    ...
    const videoInfo = await pool.request().input("id", mssql.Int, videoId)
      .query(`
                SELECT 
    ...
    if (range) {
      ...
      const end = parts[1]
        ? parseInt(parts[1], 10)
        : Math.min(start + 1024 * 1024, videoSize - 1);
      ...
      const result = await pool
        ...
        .input("length", mssql.Int, contentLength).query(`
...
exports.getVideoDetails = async (req, res) => {
  ...
  try {
    ...
    // Format the video_date for frontend display
    videoDetails.formattedDate = formatVideoDate(videoDetails.video_date);

    console.log("Video details:", videoDetails); // Add this debug log
...
exports.homePage = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id, video_name FROM videos");
...
exports.uploadVideo = async (req, res) => {
  ...
  try {
    ...
    const videoDate = new Date(); // Current timestamp
    ...
    await pool
      ...
      .input("videoDate", mssql.DateTime, videoDate) // Include video_date
      .query(`
...
exports.getComments = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("videoId", mssql.Int, videoId)
      .query(
        "SELECT * FROM comments WHERE video_id = @videoId ORDER BY comment_date DESC",
      );
...
exports.postComment = async (req, res) => {
  ...
  try {
    ...
    await pool
      ...
      .query(
        "INSERT INTO comments (video_id, user_name, comment_text, comment_date) VALUES (@videoId, @userName, @commentText, SYSDATETIME())",
      );
